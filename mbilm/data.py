...
import sys
# sys.path.append('../../software/CMU-MultimodalSDK')
...
class MultimodalDataset(object):
    def __init__(self, filepattern, vocab, test=False, shuffle_on_load=False, exclude_split=None, dataset_name="CMU_MOSEI", max_acoustic_size_per_token=200):
        ...
        self.vocab = UnicodeCharsVocabulary(
            vocab, self.max_word_length
        )

        if exclude_split is None :
            self.keys = list(self.dataset[self.text_field].keys())
        else :
            import re
            pattern = re.compile('(.*)\[.*\]')
            self.keys = []
            for key in self.dataset[self.text_field].keys() :
                segment = re.search(pattern, key).group(1)
                if segment not in exclude_split :
                    self.keys.append(key)

        print(f"{len(self.keys)} samples in multimodal dataset")
        if shuffle_on_load : 
        ...
        assert self.acoustic_dim * self.max_acoustic_size_per_token == acoustic_feature.shape[-1]

    def _to_sentence(self, string_array, reverse=False):
        if reverse :
            return ' '.join([ w[0].decode() for w in string_array[::-1]])
        else :
            return ' '.join([ w[0].decode() for w in string_array])
        
    def _get_batches(self, batch_size, num_steps, acoustic_dim=74, reverse=False, sentence_mode=False):
        ...
        # Handle muliple sentences at once
        cur_stream = [None] * batch_size
        ...
        while True : 
            ...
            inputs = np.zeros([batch_size, num_steps], np.int32)
            acoustic_inputs = np.zeros([batch_size, num_steps, self.max_acoustic_size_per_token, self.acoustic_dim], dtype=np.float64)
            ...
            for i in range(batch_size):
                ...
                while cur_pos < num_steps: 
                    ...
                    if key is None or cur_stream[i] is None or cur_stream[i][0].shape[0] < 1:
                        try : 
                            key = next(keyIter)
                            key_list.append(key)
                            cur_stream[i] = [None] * 3
                            text_sentence = self._to_sentence(self.dataset[self.text_field][key]['features'], reverse=reverse)
                            if reverse :
                                acoustic_feature = self.dataset[self.acoustic_field][key]['features'][::-1,:]
                            else :
                            ...
                            while acoustic_feature.shape[0] < len(text_sentence.split()) :
                                acoustic_feature = np.concatenate((acoustic_feature, np.expand_dims(acoustic_feature[-1], axis=0)))

                            cur_stream[i][0] = self.vocab.encode(text_sentence, reverse=reverse)
                            cur_stream[i][1] = self.vocab.encode_chars(text_sentence, reverse=reverse)
                            cur_stream[i][2] = np.concatenate((np.expand_dims(acoustic_feature[0], axis=0), acoustic_feature, np.expand_dims(acoustic_feature[-1],axis=0)))
                    ...
                    try:
                        how_many = min(cur_stream[i][0].shape[0], num_steps - cur_pos)
                        next_pos = cur_pos + how_many
                        ...
                        acoustic_inputs[i, cur_pos:next_pos] = cur_stream[i][2][:how_many]

                        if cur_stream[i][0].shape[0] - 1 < how_many :
                            # Not enough left in cur_stream to shift by 1; just fill in with the rest
                            targets[i, cur_pos:(cur_pos + cur_stream[i][0].shape[0]-1)] = cur_stream[i][0][1:]
                        else :
                            targets[i, cur_pos:next_pos] = cur_stream[i][0][1:how_many+1]
            ...
            assert not np.any(np.isnan(acoustic_inputs)), "Acoustic features has NaN"

            X = { 'keys':key_list, 
                  'token_ids':inputs, 
                  'tokens_characters': char_inputs,
                  'tokens_acoustic': acoustic_inputs,
                  'next_token_id': targets
                    }
    ...
    def iter_batches(self, batch_size, num_steps, shuffle_on_load=False, sentence_mode=False):
        
        if shuffle_on_load : 
            random.shuffle(self.keys)

        for X, Xr in zip(self._get_batches(batch_size, num_steps, reverse=False, sentence_mode=False), 
                         self._get_batches(batch_size, num_steps, reverse=True, sentence_mode=False)) : 
            
            assert np.all([k1 == k2 for k1, k2 in zip(X['keys'], Xr['keys'])]), "Forward and backward keys are different {}".format([ (k1, k2) for k1, k2 in zip(X['keys'], Xr['keys']) if k1 != k2])
            for k, v in Xr.items():
                if k is not 'keys' : 
                    X[k + '_reverse'] = v
    ...
    def iter_sentences(self, batch_size, num_steps, shuffle_on_load=False):


        def _get_features(key, reverse=False):
            text_sentence = self._to_sentence(self.dataset[self.text_field][key]['features'], reverse=reverse)
            acoustic_feature = self.dataset[self.acoustic_field][key]['features']
            ...
            word_feature = self.vocab.encode(text_sentence, reverse=reverse)
            char_feature= self.vocab.encode_chars(text_sentence, reverse=reverse)
            acoustic_feature = np.concatenate((np.expand_dims(acoustic_feature[0], axis=0), acoustic_feature, np.expand_dims(acoustic_feature[-1],axis=0)))

            label_feature = self.dataset[self.label_field][key]['features']


            return word_feature, char_feature, acoustic_feature, label_feature
        ...
        while True :
            inputs = np.zeros([batch_size, num_steps], np.int32)
            acoustic_inputs = np.zeros([batch_size, num_steps, self.max_acoustic_size_per_token, self.acoustic_dim], dtype=np.float64)
            char_inputs = np.zeros([batch_size, num_steps, self.max_word_length], np.int32)

            inputs_rev = np.zeros([batch_size, num_steps], np.int32)
            acoustic_inputs_rev = np.zeros([batch_size, num_steps, self.max_acoustic_size_per_token, self.acoustic_dim], dtype=np.float64)
            char_inputs_rev = np.zeros([batch_size, num_steps, self.max_word_length], np.int32)

            key_list = [None] * batch_size
            ...
            for i in range(batch_size):
                try : 
                    ...
                    word_feature, char_feature, acoustic_feature, label_feature = _get_features(key, reverse=reverse)
                    label_list[i] = label_feature

                    how_many = word_feature.shape[0]
                    assert how_many <= num_steps, "Length of sentence ({}) is longer than predefined number of steps ({})".format(how_many, num_steps)
                    len_list[i] = how_many

                    inputs[i, 0:how_many] = word_feature
                    char_inputs[i, 0:how_many] = char_feature
                    acoustic_inputs[i, 0:how_many] = acoustic_feature
                    ...
                    inputs_rev[i, 0:how_many] = word_feature
                    char_inputs_rev[i, 0:how_many] = char_feature
                    acoustic_inputs_rev[i, 0:how_many] = acoustic_feature
                except StopIteration:
                    no_more_data = True
                    break
            ...
            X = { 'keys':key_list, 
                  ...
                  'tokens_characters': char_inputs,
                  'tokens_acoustic': acoustic_inputs,
                  'token_ids_reverse':inputs_rev, 
                  'tokens_characters_reverse': char_inputs_rev,
                  'tokens_acoustic_reverse': acoustic_inputs_rev,
                  'next_token_id': inputs,
                  'next_token_id_reverse': inputs_rev,
                  'lengths': len_list,
                  'labels_emotions': label_list
                    }

            yield X

            if no_more_data:
                print("End of dataset ({} sentences)".format(total_sentences))
                return
